# LocalAI 统一架构设计方案
## 第一阶段架构设计文档 v2.1-P0（P0修复版）
### —— 单一二进制交付 × 统一前端体验 × 本地优先安全 × 双层背压优化 × Agent池化会话管理 × 线程安全增强
...
| **本次迭代重点** | 🔴线程安全修复 / 🔴协议版本协商 / 🔴Agent并发约束明确 / SQLite WAL / Windows ConPTY |
...
| **🔴 Thread-Safe Callback** | P0: 确保 WebSocket 操作在主线程执行 | 所有 `ws->send()` 通过 `uWS::Loop::defer()` 抛回事件循环线程 | 基于 [uWebSockets#threading](https://github.com/uNetworking/uWebSockets/blob/master/misc/READMORE.md#threading) |
| **🔴 Protocol Versioning** | P0: 前后端协议版本协商防静默错误 | WebSocket 握手阶段交换 `protocol_version` + `capabilities` | 基于 [RFC 6455§1.9](https://datatracker.ietf.org/doc/html/rfc6455#section-1.9) 子协议协商 |
| **🔴 Agent Concurrency** | P0: 明确单实例单会话约束防数据污染 | `AgentInstance` 内部加锁 + `occupied_` 标记，确保单实例单会话 | 基于 [llama.cpp#context-architecture](https://github.com/ggerganov/llama.cpp/blob/master/docs/architecture.md) KV Cache 隔离需求 |

### 1.2 双层背压设计原理（P0增强）

| 维度 | 问题 | v2.0 方案 | v2.1-P0 增强 |
|------|------|-----------|--------------|
...
| **🔴 线程安全** | llama.cpp 回调在子线程，`ws->send()` 需主线程 | 无明确约束 | 所有回调通过 `uWS::Loop::defer()` 抛回主线程 |

### 1.3 双层背压与会话管理架构图（P0修复版）
...
flowchart TB
    subgraph Frontend[ "前端 (React/CLI) " ]
        ...
        Hook --> Handshake[ "协议握手 (version=2.1) " ]
    end
    ...
    subgraph Engine[ "C++20 AI 引擎 (localai_core) " ]
        ...
        AgentPool[ "AgentPool <br/ >(CLI 可配置池大小 + 单实例单会话) " ]
        
        EventLoop[ "uWebSockets 事件循环 (主线程) " ]
        EventLoop --> MultiPath[ "多路径注册 (/stream, /pty, /fs, /config) " ]
        EventLoop --> SocketData[ "PerSocketData 状态管理 " ]
        EventLoop --> Defer[ "Loop::defer() 线程安全回调 " ]
        
        ThreadPool[ "cppcoro::static_thread_pool (工作线程) " ]
    ...
    Frontend -->| "WebSocket 连接 + Session ID + version=2.1 "| WebSocket
    WebSocket -->| "双向数据流 "| SessionMgr
    SessionMgr -->| "获取/释放 Agent (单实例单会话) "| AgentPool
    AgentPool -->| "推理任务 (子线程) "| ThreadPool
    ThreadPool -->| "Token 回调 (defer 抛回主线程) "| Defer
    Defer -->| "ws->send() (主线程安全) "| EventLoop
    ...
    style Defer fill:#fff9c4,stroke:#fbc02d,stroke-width:2px,stroke-dasharray: 3 3
```
...
flowchart TB
    subgraph Client[ "统一 Tauri 桌面客户端（本地运行） " ]
        ...
        WebView --> Handshake[ "协议握手 (version=2.1) " ]
...
| 层次 | 职责 | 技术栈 | 演进保障 | P0 增强 |
|------|------|--------|----------|---------|
| **访问层** | 系统托盘、自动启动、窗口管理、远程连接 | Tauri v2 (Optional) | 仅作为 Shell，不参与业务逻辑，可随时替换 | SSH 主机密钥策略可配置接口 |
| **前端层** | 用户交互、流式渲染、布局配置、组件管理 | React 18 + TypeScript + Vite + Zustand | 组件仅依赖 WebSocket 接口，环境无关 | `error_mapper.ts` 错误码映射层 + 协议版本握手 |
| **通信层** | 统一 WebSocket 协议，背压控制，多路径路由，错误限流 | uWebSockets v20+ | 全双工通信，支持 `/stream`, `/pty`, `/fs`, `/config` | 解析错误计数器 + 连接保护 + `Loop::defer()` 线程安全 |
| **引擎层** | 推理执行、会话管理、Agent 池化、原生 PTY、路径校验 | C++20 + SQLite + llama.cpp + cppcoro | 核心逻辑封装，支持条件编译裁剪 | 单实例单会话约束 + Windows ConPTY / SQLite WAL / 路径规范化 |
...
sequenceDiagram
    ...
    WS->>Engine: ws://127.0.0.1:PORT/stream + {"version":"2.1"}
    
    Note right of WS: P0: 协议握手
    Engine->>Engine: 校验 protocol_version
    alt 版本兼容
        Engine-->>WS: {"type":"handshake_ack","version":"2.1"}
    else 版本不兼容
        Engine-->>WS: {"error":{"code":0x0002,"message":"Protocol version mismatch"}}
        WS-->>Frontend: 提示用户升级
        return
    end
    ...
    Note right of AgentPool: P0: 单实例单会话约束
    AgentPool-->>SessionMgr: 复用/创建 Agent 实例 (occupied_=true)
    SessionMgr-->>Engine: 返回 AIProvider
    Engine->>Llama: llama_decode() (子线程)
    
    loop Token 生成
        Llama-->>Engine: Token (子线程回调)
        
        Note right of Engine: P0: 线程安全 - defer 抛回主线程
        Engine->>Engine: Loop::defer([ws, chunk]{ ws->send(json) })
        Engine-->>WS: ws->send(json) (主线程安全)
        WS-->>Frontend: 更新 UI
    ...
    Note right of SessionMgr: P0: 释放会话时归还 Agent (occupied_=false)
    SessionMgr->>AgentPool: release_agent()
```

### 2.4 会话与 Agent 池化架构图（P0修复版）
...
flowchart TB
    ...
    subgraph AgentPoolDetail[ "Agent 实例池 (CLI 可配置 + 单实例单会话) " ]
        ...
        Occupancy[ "occupied_ 标记 + mutex 保护 " ]
    end
    ...
    subgraph WebSocket[ "WebSocket 连接 (错误限流 + 协议版本) " ]
        Conn1[ "连接 1 <br/>session_id=abc<br/>version=2.1 " ]
        Conn2[ "连接 2 <br/>session_id=def<br/>version=2.1 " ]
        Conn3[ "连接 3 <br/>session_id=abc<br/>version=2.1 " ]
    end
    ...
    style Occupancy fill:#ffebee,stroke:#c62828,stroke-width:1px,stroke-dasharray: 3 3
```

### 2.5 远程访问架构（SSH 隧道，P0增强）
...
flowchart LR
    ...
    style VersionCheck fill:#fff9c4,stroke:#fbc02d,stroke-width:1px,stroke-dasharray: 3 3
```
...
// src/providers/local_cxx_provider.ts (P0: 握手逻辑)
export class LocalCxxProvider implements AIProvider {
  async connect(url: string, config: ConnectionConfig): Promise<void> {
      ...
      this.ws.onmessage = (event) => {
        ...
        if (msg.type === "handshake_ack") {
          if (msg.version !== PROTOCOL_VERSION) {
            reject(new Error(`Protocol version mismatch: expected ${PROTOCOL_VERSION}, got ${msg.version}`));
            return;
          }
          // 协商背压参数
          if (msg.negotiated?.ack_batch_size) {
            this.ackBatchSize = msg.negotiated.ack_batch_size;
          }
          resolve();
        } else if (msg.error) {
...
export const ERROR_CODE_MAP: Record<number, ErrorDisplay> = {
  ...
  0x0002: {  // P0: 协议版本不匹配
    title: "协议版本不兼容",
    message: "客户端与后端协议版本不一致",
    actionable: true,
    suggestion: "请升级 LocalAI 到最新版本"
  },
  0x1001: { 
    title: "内存不足", 
...
// P0: 类型守卫确保运行时安全
export function isValidErrorCode(code: number): code is keyof typeof ERROR_CODE_MAP {
...
//     }
//   } else {
//     toast.error("未知错误", { description: error.message });
//   }
// }
```
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // P0: 协议版本错误特殊处理
        if (err instanceof Error && err.message.includes('Protocol version')) {
          toast.error("版本不兼容", { 
            description: "请升级 LocalAI 客户端或后端",
            action: { label: "检查更新", onClick: () => window.open('https://github.com/...') }
          });
        }
      }
    };
...
int main(int argc, char** argv) {
    ...
    // 创建 Agent 池（v2.1: 传入可配置参数 + P0: 单实例单会话约束）
...
#### 3.2.2 会话管理器（SessionManager，P0增强）
...
public:
    ...
    void release_session(const std::string& session_id) {
        auto it = session_map_.find(session_id);
        if (it != session_map_.end()) {
            // P0: 释放时归还 Agent 到池 (标记 occupied_=false)
...
private:
    ...
    void init_schema() {
        ...
        // P0: 禁用自动 checkpoint，避免高频写入时锁竞争
        db_->exec("PRAGMA wal_autocheckpoint=0;");
...
#### 3.2.3 Agent 实例池（AgentPool，P0单实例单会话约束）
...
// P0: Agent 实例包装器，确保单实例单会话
class AgentInstance {
public:
    AgentInstance(std::shared_ptr<AIProvider> provider) 
        : provider_(std::move(provider)), occupied_(false) {}
    
    // P0: 获取实例（检查占用状态）
    std::shared_ptr<AIProvider> acquire() {
        std::lock_guard<std::mutex> lock(mutex_);
        if (occupied_) {
            return nullptr;  // 已被其他会话占用
        }
        occupied_ = true;
        return provider_;
    }
...
public:
    ...
    std::shared_ptr<AIProvider> get_agent(const std::string& agent_type) {
        ...
        // P0: 遍历池寻找空闲实例
        for (auto& wrapper : pool) {
            if (!wrapper->is_occupied()) {
                auto provider = wrapper->acquire();
                if (provider) {
                    return provider;  // 成功获取空闲实例
                }
            }
        }
        ...
        if (get_total_count() < config_.max_total_agents && 
            ...
            auto new_provider = create_new_agent(agent_type);
            auto wrapper = std::make_shared<AgentInstance>(new_provider);
            wrapper->acquire();  // 新实例立即标记为占用
            pool.push_back(wrapper);
            return new_provider;
        }
    ...
    void release_agent(const std::string& agent_type, std::shared_ptr<AIProvider> agent) {
        ...
        for (auto& wrapper : pool) {
            // 通过指针比较找到对应实例
            // 实际实现中可维护 agent->wrapper 的反向映射以提高效率
            if (wrapper->acquire() == agent) {  // acquire 会检查 occupied_
                wrapper->release();  // 标记为空闲
                return;
            }
        }
    }
...
private:
    ...
    std::unordered_map<std::string, std::vector<std::shared_ptr<AgentInstance>>> pools_;
...
#### 3.2.4 双层背压集成 + 错误限流 + 线程安全（P0核心修复）

```cpp
// engine-cpp/src/handlers/stream_handler.hpp (关键片段)
namespace StreamHandler {

// P0: PerSocketData 增加协议版本与错误限流字段
struct PerSocketData {
    std::string connection_id;
    std::string session_id;
    std::string protocol_version;  // P0: 协商后的协议版本
    bool handshake_completed = false;  // P0: 握手完成标记
...
inline auto create_handlers(std::shared_ptr<SessionManager> session_manager) {
    return uWS::App::WebSocketBehavior<PerSocketData>{
        .open = [session_manager](auto* ws) {
            ...
            // P0: 初始化为未知版本，握手后更新
            data->protocol_version = "unknown";
            data->handshake_completed = false;
        },
        ...
        .message = [session_manager](auto* ws, std::string_view message, uWS::OpCode opCode) {
            ...
            if (!data->handshake_completed) {
                if (doc["type"].get_string().value_or("") == "handshake") {
                    auto client_version = doc["version"].get_string().value_or("1.0");
                    
                    // P0: 版本兼容性检查
                    if (client_version != "2.1") {
                        // 返回错误码，前端可映射
                        ws->send(R"({ "error": { "code": 0x0002, "message": "Protocol version mismatch" }})");
                        ws->close(1008, "Protocol version mismatch");  // 1008 = policy violation
                        return;
                    }
                    
                    // 记录协商结果
                    data->protocol_version = client_version;
                    data->handshake_completed = true;
                    
                    // 解析背压参数
                    if (doc["backpressure"].is_object()) {
                        auto bp = doc["backpressure"];
                        if (bp["ack_batch_size"].get_uint64().value_or(0) > 0) {
                            data->ack_batch_size = bp["ack_batch_size"].get_uint64().value();
                        }
                        if (bp["transport_high_water"].get_uint64().value_or(0) > 0) {
                            data->transport_high_water = bp["transport_high_water"].get_uint64().value();
                        }
                        if (bp["app_high_water"].get_uint64().value_or(0) > 0) {
                            data->app_high_water = bp["app_high_water"].get_uint64().value();
                        }
                    }
                    
                    // 返回握手确认
                    auto resp = simdjson::minify(simdjson::padded_string(
                        R"({ "type": "handshake_ack", "version": "2.1", "negotiated": {)" +
                        R"("ack_batch_size": )" + std::to_string(data->ack_batch_size) + R"(,)" +
                        R"("transport_high_water": )" + std::to_string(data->transport_high_water) + R"(,)" +
                        R"("app_high_water": )" + std::to_string(data->app_high_water) + R"(}})"
                    ));
                    ws->send(resp, uWS::OpCode::TEXT);
                    return;
                } else {
                    // 未握手先发消息，拒绝
                    ws->send(R"({ "error": { "code": 0x0003, "message": "Handshake required first" }})");
                    return;
                }
            }
            ...
            thread_pool::get().submit([ws, params, session_manager]() mutable {
                ...
                agent->stream_completion(
                    ...
                    [ws, session_manager, session_id = params.session_id](const TokenChunk& chunk) {
                        // P0: 线程安全 - 必须通过 defer 抛回主线程
                        // 依据: uWebSockets READMORE#threading - ws 操作必须在创建它的 Loop 线程执行
                        auto* loop = uWS::Loop::get();
                        loop->defer([ws, session_manager, session_id, chunk]() {
                            auto* data = ws->getUserData();
...
inline std::optional<std::string> safe_join(
    ...
    // P0: 更严谨的边界检查
    if (full_path == norm_base) return full_path;  // 精确匹配
    if (full_path.length() <= norm_base.length()) return std::nullopt;
    if (full_path[norm_base.length()] != '/') return std::nullopt;  // 必须是子目录
    if (!full_path.starts_with(norm_base + "/")) return std::nullopt;
...
### 3.3 通信协议：纯 WebSocket（P0协议版本增强）

#### 3.3.1 路径定义与消息格式

| 路径 | 功能 | 请求示例 | 响应示例 | P0 增强 |
|------|------|----------|----------|---------|
...
| `/handshake` | P0 新增: 协议版本协商 | `{"type":"handshake","version":"2.1","capabilities":[...]}` | `{"type":"handshake_ack","version":"2.1","negotiated":{...}}` | 版本不匹配返回 `0x0002` |

#### 3.3.2 应用层 ACK 协议（参数可配置）

```json
// P0: 握手时协商协议版本与背压参数（必需）
{
  "type": "handshake",
  "version": "2.1",
  "capabilities": ["backpressure:configurable", "multimodal", "pty:conpty"],
  "backpressure": {
    "ack_batch_size": 20,
...
set(SOURCES
    ...
    src/session/agent_pool.cpp        # v2.1: CLI 配置支持 + P0 单实例单会话
...
#    - 前端自动发起 protocol_version=2.1 握手
#    - 版本不匹配时显示友好升级提示
```
...
| 风险 | v2.0 防御措施 | v2.1-P0 增强措施 | 技术依据 |
|------|--------------|-----------------|----------|
| **数据外泄** | 强制绑定 `127.0.0.1` + SSH 隧道加密 | 同左 + 路径规范化校验 | `uSockets::bind()` + SSH 协议 + `realpath(3)` |
| **文件越权** | 后端校验路径是否在 `allowed_dirs` 内 | `safe_join()` 函数 + 符号链接检测 + 边界检查强化 | CWE-22, `realpath(3)` man page |
...
| **会话劫持** | 会话 ID 随机生成 + SQLite 权限控制 | 同左 + 错误状态同步防重放 + 协议版本校验 | `SessionManager` 内部映射 |
| **线程竞争** | 无 | `uWS::Loop::defer()` 确保 ws 操作主线程执行 + Agent 单实例单会话约束 | [uWebSockets#threading](https://github.com/uNetworking/uWebSockets/blob/master/misc/READMORE.md#threading) |
| **协议漂移** | 无 | 握手阶段强制版本协商 + 不兼容时明确拒绝 | [RFC 6455§1.9](https://datatracker.ietf.org/doc/html/rfc6455#section-1.9) |
...
enum class ErrorCode : uint32_t {
    ...
    PROTOCOL_VERSION_MISMATCH = 0x0002,  // P0: 协议版本不兼容
    HANDSHAKE_REQUIRED = 0x0003,         // P0: 未握手先发消息
...
### Phase 1.1-P0: 核心修复（4-5 周）

```mermaid
gantt
    title Phase 1.1-P0 实施计划 (v2.1 含 P0 修复)
    dateFormat  YYYY-MM-DD
    section P0 核心修复
    线程安全: Loop::defer 集成    :active,  thread_safe1, 2026-03-10, 3d
    协议版本: 握手协商实现        :         proto_ver1, 2026-03-10, 3d
    Agent 约束: 单实例单会话      :         agent_lock1, 2026-03-13, 3d
    
    section 后端增强
    SQLite WAL 模式集成          :         wal1, 2026-03-10, 3d
    ...
    协议握手 UI 集成            :         handshake_ui1, 2026-03-13, 2d
    ...
    section 测试验证
    线程安全并发测试            :         thread_test1, 2026-03-27, 4d
    协议版本兼容性测试          :         proto_test1, 2026-03-27, 3d
...
# 使用高并发客户端同时发起推理请求
# 验证无崩溃、无数据错乱、无断连
# 工具: wrk + 自定义 WebSocket 客户端

# 🔴 P0: 协议版本验证
# 1. 使用 version=2.0 客户端连接 -> 应返回 0x0002 错误
# 2. 使用 version=2.1 客户端连接 -> 应成功握手
# 3. 握手后发送推理请求 -> 应正常响应

# 🔴 P0: Agent 并发约束验证
# 1. 创建 2 个会话使用同一 Agent 类型
# 2. 验证两个会话的推理输出不交叉污染
# 3. 验证池耗尽时返回 0x5001 而非数据错乱
```
...
| **🔴 线程安全** | P0: 所有 WebSocket 操作通过 `defer` 抛回主线程 | [uWebSockets#threading](https://github.com/uNetworking/uWebSockets/blob/master/misc/READMORE.md#threading) |
| **🔴 协议稳定** | P0: 握手阶段强制版本协商，防止静默兼容错误 | [RFC 6455§1.9](https://datatracker.ietf.org/doc/html/rfc6455#section-1.9) |
| **🔴 数据隔离** | P0: Agent 单实例单会话约束，防止多会话数据污染 | [llama.cpp#context-architecture](https://github.com/ggerganov/llama.cpp/blob/master/docs/architecture.md) |
...
10. **🔴 线程安全**：所有 `ws->send()` 通过 `uWS::Loop::defer()` 抛回事件循环线程，避免跨线程调用未定义行为
11. **🔴 协议版本**：握手阶段强制交换 `version` + `capabilities`，不兼容时明确拒绝而非静默失败
12. **🔴 并发约束**：`AgentInstance` 包装器 + `occupied_` 标记，确保单实例单会话，防止 KV Cache 污染
...
✅ **🔴 线程安全**：`Loop::defer()` 确保 WebSocket 操作主线程执行，避免竞态条件  
✅ **🔴 协议稳定**：握手版本协商防止前后端漂移，错误时明确提示而非静默失败  
✅ **🔴 数据隔离**：Agent 单实例单会话约束，确保多会话推理输出不交叉污染  

---

## 📚 附录：关键 Repo 与接口引用（P0更新）

| 组件 | Repo | 关键文件/接口 | 用途 | P0 关联 |
|------|------|--------------|------|---------|
| **uWebSockets** | [uNetworking/uWebSockets](https://github.com/uNetworking/uWebSockets) | `src/WebSocket.h#L1150`, `misc/READMORE.md#threading` | 多路径注册 + `getBufferedAmount()` + `res->endFromFile()` + **线程安全约束** | 背压参数协商 + 错误限流 + `Loop::defer()` |
| **llama.cpp** | [ggerganov/llama.cpp](https://github.com/ggerganov/llama.cpp) | `include/llama.h#L450`, `docs/architecture.md` | `llama_decode` 线程安全约束 + 模型加载 + **KV Cache 会话隔离** | Agent 池资源评估 + 单实例单会话约束 |
| **SQLite** | [sqlite/sqlite](https://github.com/sqlite/sqlite) | `sqlite3.h`, `doc/wal.html` | 会话与配置持久化，WAL 模式 + `busy_timeout` | 并发写入优化 + checkpoint 策略 |
...
| **RFC 6455** | [ietf/rfcs](https://datatracker.ietf.org/doc/html/rfc6455) | `§1.9 Subprotocol Negotiation` | WebSocket 子协议协商标准 | 协议版本握手设计 |

---

> **文档版本**: v2.1-P0 (P0 修复版)  
> **最后更新**: 2026-03-09  
> **状态**: Phase 1.1 正式提案（含核心风险修复）  
> **维护者**: LocalAI Architecture Team  
> **评审原则**: 任何技术结论均依据开源仓库接口规范与内核行为推导，拒绝主观臆想  
> **P0 修复摘要**:  
> 1. 🔴 线程安全: 所有 `ws->send()` 通过 `uWS::Loop::defer()` 抛回主线程  
> 2. 🔴 协议版本: 握手阶段强制交换 `version` + `capabilities`，不兼容时明确拒绝  
> 3. 🔴 并发约束: `AgentInstance` 包装器 + `occupied_` 标记，确保单实例单会话